...
import findFixedTransport from "../controllers/transports/fixed/findFixedTransport.controller";
...
const PrintFixedTransport = () => {
    const { id } = useParams();
    const componentRef = useRef<HTMLDivElement>(null);
    const [transportData, setTransportData] = useState<FixedTransport>({} as FixedTransport);
    const [loaded, setLoaded] = useState(false);
    const [driversData, setDriversData] = useState<Driver[]>([]);
    const [vehiclesData, setVehiclesData] = useState<Vehicle[]>([]);

    if (!loaded) {
        findFixedTransport(id as string)
            .then((data) => {
...
export default PrintFixedTransport;
